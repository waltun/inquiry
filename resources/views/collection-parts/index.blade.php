        ...
        <ol class="inline-flex items-center space-x-2 space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('collections.parts',$collection->id) }}"
                       class="mr-2 text-xs md:text-sm font-medium text-gray-500 hover:text-gray-900">
                        لیست قطعات مجموعه {{ $collection->name }}
                    </a>
                </div>
            </li>
    ...
    <div class="mt-4 flex justify-between space-x-4 space-x-reverse">
        <div>
            <p class="text-lg font-bold text-black">
                افزودن قطعه به مجموعه <span class="text-red-600">{{ $collection->name }}</span>
            </p>
        </div>
        <div>
    ...
    <div
        ...
        <div class="bg-white p-4 shadow-md rounded-md border border-gray-200">
            ...
            @if(request()->has('code') || request()->has('search'))
                <div class="mt-4">
                    <a href="{{ route('collections.parts.index',$collection->id) }}" class="form-detail-btn text-xs">
                        پاکسازی جستجو
                    </a>
                </div>
            @endif
                ...
                @foreach($parts as $part)
                    <tr>
                        ...
                        <td class="px-4 py-3 space-x-3 space-x-reverse">
                            <form action="{{ route('collections.parts.store',[$collection->id,$part->id]) }}"
                                  method="POST" class="inline">
                                @csrf
                                <button class="form-submit-btn text-xs">
                                    افزودن به مجموعه {{ $collection->name }}
                                </button>
                            </form>
                        </td>
                    </tr>
