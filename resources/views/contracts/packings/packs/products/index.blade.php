<x-layout>
        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="font-bold text-2xl text-black dark:text-white">
                    لیست محصولات پک {{ $pack->name }}
                </p>
            </div>
        </div>
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('packs.index', [$contract->id, $pack->packing->id]) }}" class="page-warning-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
    ...
    <div class="mt-4 space-y-4">
        ...
        @foreach($types as $type)
            @php
                $products = $pack->products()->where('part_id','!=',0)->where('type',$type)->get();
            @endphp
            @if(!$products->isEmpty())
                <div class="card">
        ...
        <div>
            <a href="{{ route('contracts.packs.products.create', [$contract->id, $pack->id]) }}"
