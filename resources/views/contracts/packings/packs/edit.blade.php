<x-layout>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('packs.update', [$contract->id, $packing->id, $pack->id]) }}" class="mt-4">
        @csrf
        @method('PATCH')
            ...
            <div class="mb-4 grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="name" id="inputName" class="input-text">
                        ...
                        @foreach($names as $name)
                            <option value="{{ $name }}" {{ old('name', $pack->name) == $name ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                        @if(!in_array($pack->name, $names->toArray()))
                            <option
                                    value="{{ $pack->name }}" {{ old('name', $pack->name) == $pack->name ? 'selected' : '' }}>
                                {{ $pack->name }}
                            </option>
                        @endif
                        <option
                ...
                <div>
                    ...
                    <select name="type" id="inputType" class="input-text">
                        <option value="">انتخاب یا وارد کنید</option>
                        <option
                                value="کاور نایلون حباب دار با تسمه بندکشی" {{ old('type', $pack->type) == 'کاور نایلون حباب دار با تسمه بندکشی' ? 'selected' : '' }}>
                            کاور نایلون حباب دار با تسمه بندکشی
                        </option>
                        <option
                                value="پالت چوبی با روکش نایلون حباب دار و تسمه بندکشی" {{ old('type', $pack->type) == 'پالت چوبی با روکش نایلون حباب دار و تسمه بندکشی' ? 'selected' : '' }}>
                            پالت چوبی با روکش نایلون حباب دار و تسمه بندکشی
                        </option>
                        <option value="کارتن" {{ old('type', $pack->type) == 'کارتن' ? 'selected' : '' }}>
                            کارتن
                        </option>
