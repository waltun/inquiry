<x-layout>
    ...
    <div class="flex items-center justify-between mt-8">
        ...
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('packs.create', [$contract->id, $packing->id]) }}" class="page-success-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    ...
                    <tr class="table-tb-tr group whitespace-normal {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            <div class="flex justify-center items-center">
                                <a href="{{ route('contracts.packs.products.index', [$contract->id, $pack->id]) }}" class="table-warning-btn">
                                    ...
                                    مشاهده ({{ count($pack->products) }})
                                </a>
                            </div>
                        </td>
                        <td class="table-tr-td border-t-0 border-r-0 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-4 space-x-reverse">
                                <a href="{{ route('packs.marking',[$contract->id, $packing->id, $pack->id]) }}" target="_blank"
                                ...
                                <a href="{{ route('packs.edit',[$contract->id, $packing->id, $pack->id]) }}"
                                ...
                                <form action="{{ route('packs.destroy', [$contract->id, $packing->id, $pack->id]) }}"
                                      method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="table-delete-btn" onclick="return confirm('پک حذف شود ؟')">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                             stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-1">
                                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="mt-4 flex justify-end">
        <a href="{{ route('packings.show', [$contract->id, $packing->id]) }}" target="_blank"
           class="text-indigo-500 underline-offset-4 underline font-medium flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            برای پرینت لیست کامل این پکینگ لیست کلیک کنید
        </a>
    </div>
</x-layout>
