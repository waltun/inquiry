<x-layout>
    ...
    <div class="my-4">
        <form method="POST" action="{{ route('contracts.calculateExastDamper', $contract->id) }}"
    ...
    <!-- Content -->
    @if(!is_null($values))
        <form method="POST" action="{{ route('calculateDamper.store', [$contract->id, $part->id, $product->id, $part2->id]) }}" class="mt-4">
            @csrf
