        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="font-bold text-2xl text-black dark:text-white">
                    ویرایش دستور ساخت قرارداد {{ $contract->name }} - CNT-{{ $contract->number }}
                </p>
            </div>
        </div>
        ...
        @if(!$products->isEmpty())
            @foreach($products as $product)
                @php
                    ...
                    $weight = 0;
                @endphp
                <form method="POST" action="{{ route('contracts.recipe.store-parts', $contract->id) }}" class="card">
                    @csrf
