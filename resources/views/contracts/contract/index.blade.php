<x-layout>
    ...
    @if(is_null($contract->file))
        <form method="POST" action="{{ route('contracts.contract.store', $contract->id) }}" class="mt-4">
            @csrf
            @method('PATCH')
        ...
        <div class="mt-4 bg-green-500 p-4 rounded-md shadow">
            <div class="mb-4">
                <p class="text-sm font-bold text-white text-center">
                    فایل قرارداد با موفقیت بارگذاری شده است، برای دانلود روی لینک زیر کلیک کنید.
                </p>
            </div>
            <div class="flex items-center justify-center">
                <a href="{{ $contract->file }}" class="text-sm font-bold text-indigo-500 underline underline-offset-4"
                   download>
                    دانلود فایل قرارداد
                </a>
            </div>
