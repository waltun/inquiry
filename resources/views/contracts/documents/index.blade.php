        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="font-bold text-2xl text-black dark:text-white">
                    لیست مدارک تایید شده قرارداد {{ $contract->name }}
                </p>
            </div>
        </div>
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('documents.create', $contract->id) }}" class="page-success-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                @foreach($documents as $document)
                    <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ jdate($document->date)->format('Y/m/d') }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ $document->name }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0">
                            ...
                            @if($extension[1] == 'pdf' || $extension[1] == 'docx' || $extension[1] == 'doc')
                                <div class="flex items-center justify-center">
                                    <a href="{{ $document->file }}" class="table-dropdown-copy" download>
                            ...
                            @elseif($extension[1] == 'jpg' || $extension[1] == 'png' || $extension[1] == 'jpeg')
                                <img src="{{ $document->file }}" alt="" class="w-10 h-10 rounded-md mx-auto border border-gray-200 cursor-pointer"
                        ...
                        <td class="table-tr-td border-t-0 border-r-0 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-4 space-x-reverse">
                                ...
                                <form action="{{ route('documents.destroy',[$contract->id, $document->id]) }}" method="POST" class="table-dropdown-delete">
                                    @csrf
                                    @method('DELETE')
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-1">
                                    ...
                                    <button onclick="return confirm('مدرک تایید شده حذف شود ؟')">
                                        حذف
                                    </button>
                                </form>
                            </div>
