<x-layout>
    <x-slot name="js">
        ...
        <script>
            function destroyMarketing(id) {
                if (confirm('بازاریابی حذف شود ؟')) {
                    $.ajaxSetup({
                        headers: {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ route('contracts.marketings.destroy') }}',
                        data: {
                            id: id
                        },
                        success: function () {
                    ...
                    <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-r-0 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-4 space-x-reverse">
                                ...
                                @if(!$marketing->confirm)
                                    <button type="button" onclick="destroyMarketing({{ $marketing->id }})"
