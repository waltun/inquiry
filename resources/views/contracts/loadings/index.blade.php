        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="breadcrumb-p-active">
                    مدیریت فایل های بارگیری و حمل {{ $contract->name }}
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="font-bold text-2xl text-black dark:text-white">
                    لیست فایل های بارگیری و حمل قرارداد {{ $contract->name }}
                </p>
            </div>
        </div>
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('loadings.create', $contract->id) }}" class="page-success-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="w-4 h-4 ml-1">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
                ایجاد فایل بارگیری جدید
            </a>
                ...
                @foreach($loadings as $loading)
                    <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ jdate($loading->date)->format('Y/m/d') }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ $loading->number }}
                        </td>
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            @php
                                $extension = explode('.',$loading->file);
                            @endphp
                            @if($extension[1] == 'pdf' || $extension[1] == 'docx' || $extension[1] == 'doc')
                                <div class="flex items-center justify-center">
                                    <a href="{{ $loading->file }}" class="table-dropdown-copy" download>
                            ...
                            @elseif($extension[1] == 'jpg' || $extension[1] == 'png' || $extension[1] == 'jpeg')
                                <img src="{{ $loading->file }}" alt="" class="w-10 h-10 rounded-md mx-auto border border-gray-200 cursor-pointer"
                        ...
                        <td class="table-tr-td border-t-0 border-r-0 whitespace-nowrap">
                            <div class="flex items-center justify-center space-x-4 space-x-reverse">
                                <a href="{{ route('loadings.edit',[$contract->id, $loading->id]) }}"
                                ...
                                <form action="{{ route('loadings.destroy',[$contract->id, $loading->id]) }}" method="POST" class="table-dropdown-delete">
                                    @csrf
                                    @method('DELETE')
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-1">
                                    ...
                                    <button onclick="return confirm('فایل بارگیری حذف شود ؟')">
                                        حذف
                                    </button>
                                </form>
                            </div>
