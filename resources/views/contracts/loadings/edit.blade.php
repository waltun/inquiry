<x-layout>
    ...
    <div class="flex items-center space-x-2 space-x-reverse whitespace-nowrap">
        ...
        <a href="{{ route('contracts.guarantees.index', $contract->id) }}" class="flex items-center">
            ...
            <div class="mr-2">
                <p class="breadcrumb-p">
                    مدیریت مدارک بارگیری و حمل {{ $contract->name }}
                </p>
            </div>
        </a>
        <div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('loadings.update', [$contract->id, $loading->id]) }}" class="mt-4 grid grid-cols-2 gap-4" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        ...
        <div class="card">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="inputNumber" name="number" class="input-text" value="{{ old('number', $loading->number) }}"
                       placeholder="شماره سند را در صورت لزوم وارد کنید">
            </div>
            <div class="mb-4">
                <label for="inputType" class="form-label">
                    نوع سند
                </label>
                <input type="text" id="inputType" name="type" class="input-text" value="{{ old('type', $loading->type) }}"
                       placeholder="نوع سند را در صورت وارد کنید">
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="mb-4">
                ...
                <input type="file" class="input-file" id="inputFile" name="file" value="{{ old('file', $loading->file) }}">
            </div>
