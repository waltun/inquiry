    ...
    <div class="md:flex items-center justify-between mt-8 space-y-4 md:space-y-0">
        <div class="flex items-center">
            ...
            <div class="mr-2 flex items-center">
                <p class="font-bold text-2xl text-black dark:text-white ml-2">
    ...
    <div class="mt-4 space-y-4">
        ...
        <div class="p-4 rounded-md shadow border border-indigo-400 bg-white">
            ...
            <div class="grid grid-cols-6 gap-4">
                ...
                @if(!$contract->products->isEmpty() && $contract->type == 'official')
                    <a href="{{ route('contracts.exclusive-code.index', $contract->id) }}"
                       class="p-2 rounded-2xl shadow border border-gray-300 {{ $contract->products->contains('code', null) ? 'bg-opacity-50 border-opacity-50 bg-gray-300' : 'bg-green-400' }}">
                        <div class="flex items-center justify-between border-b border-white pb-2">
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                ...
                                <div class="mr-2">
                                    <p class="font-bold text-black text-xs {{ $contract->products->contains('code', null) ? 'text-opacity-40' : '' }}">
                                        اخذ کد اختصاصی
                                    </p>
                                </div>
                            </div>
                            <div>
                                @if($contract->products->contains('code', null))
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                                         class="w-5 h-5 text-gray-600 text-opacity-40">
                                        <path fill-rule="evenodd"
                                              d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z"
                ...
                @if($contract->type == 'official')
                    <a href="{{ route('main-factors.index', $contract->id) }}"
                       class="p-2 rounded-2xl shadow border border-gray-300 {{ $contract->factors->isEmpty() ? 'bg-opacity-50 border-opacity-50 bg-gray-300' : 'bg-green-400' }}">
                        <div class="flex items-center justify-between border-b border-white pb-2">
                            <div class="flex items-center">
