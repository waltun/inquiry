    ...
    <div class="flex items-center">
        ...
        <a href="{{ route('notifications.index') }}" class="pl-4 relative">
            ...
            @if(count(auth()->user()->unreadNotifications) > 0)
                <span class="absolute -top-1 h-4 w-4 rounded-full bg-yellow-500 opacity-90 text-xs text-center">
                    {{ count(auth()->user()->unreadNotifications) }}
                </span>
                <span class="animate-ping absolute -top-1 h-4 w-4 rounded-full bg-yellow-500 opacity-90"></span>
            @endif
        </a>
