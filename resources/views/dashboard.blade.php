<x-layout>
    ...
    @php
        $uncompletedTasks = \App\Models\Task::where('receiver_id', auth()->user()->id)
            ->where(function($query) {
                $query->whereNull('done_at')
                    ->where(function($query) {
                        $query->whereNull('extension_days_request_at')
                              ->orWhere(function($query) {
                                  $query->whereRaw('DATE_ADD(extension_days_request_at, INTERVAL extension_days_request DAY) <= NOW()');
                              });
                    });
            })
            ->get();
        ...
        <div class="overflow-x-auto rounded-lg">
            <table class="w-full border-collapse">
                ...
                @foreach($uncompletedTasks as $uncompletedTask)
                    <tr class="table-tb-tr whitespace-normal group hover:font-bold hover:text-red-600 {{ $loop->even ? 'bg-sky-100' : '' }}">
                        <td class="table-tr-td border-t-0 border-l-0">
                            {{ jdate($uncompletedTask->date)->format('Y/m/d') }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0 {{ $uncompletedTask->done ? 'line-through opacity-50' : '' }}">
                            {{ $uncompletedTask->title }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0 {{ $uncompletedTask->done ? 'line-through opacity-50' : '' }}">
                            {{ $uncompletedTask->user->name }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0 {{ $uncompletedTask->done ? 'line-through opacity-50' : '' }}">
                            @switch($uncompletedTask->level)
                                @case('high')
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            @php
                                $extension = explode('.',$uncompletedTask->file);
                            @endphp
                            @if(!is_null($uncompletedTask->file))
                                @if($extension[1] == 'pdf' || $extension[1] == 'docx' || $extension[1] == 'doc')
                                    <div class="flex justify-center items-center">
