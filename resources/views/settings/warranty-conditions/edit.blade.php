<x-layout>
    ...
    <div class="flex items-center space-x-2 space-x-reverse">
        ...
        <a href="{{ route('settings.warrantyCondition.index') }}" class="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                 stroke="currentColor" class="breadcrumb-svg">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="breadcrumb-p-active">
                    ویرایش شرایط {{ $warrantyCondition->name }}
                </p>
            </div>
        </div>
    </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('settings.warrantyCondition.update', $warrantyCondition->id) }}" class="mt-4">
        @csrf
        @method('PATCH')
        ...
        <div class="card">
            ...
            <div class="mt-4">
                <label for="inputName" class="form-label">نام توضیحات</label>
                <input type="text" class="input-text" name="name" id="inputName"
                       placeholder="مثال : شرایط فروش فن کویل ها" value="{{ $warrantyCondition->name }}">
            </div>

            <div class="mt-4">
                <label for="inputDescription" class="form-label">توضیحات و شرایط گارانتی</label>
                <textarea name="description" id="inputDescription">{{ $warrantyCondition->description }}</textarea>
            </div>

        </div>

        <div class="mt-4 flex space-x-4 space-x-reverse items-center">
            <button type="submit" class="form-edit-btn">
                بروزرسانی شرایط
            </button>
            <a href="{{ route('settings.warrantyCondition.index') }}" class="form-cancel-btn">
                انصراف
            </a>
        </div>
    </form>
