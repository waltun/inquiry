<x-layout>
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="mr-2 text-xs md:text-sm font-medium text-gray-400">
                        تعیین مقادیر مجموعه {{ $collection->name }}
                    </span>
                </div>
            </li>
        </ol>
    ...
    <!-- Content -->
    <form method="POST" action="{{ route('collections.storeAmounts',$collection->id) }}" class="mt-4">
        @csrf

        <div class="bg-white shadow-md border border-gray-200 rounded-md py-4 px-6 mb-4">
            <div class="md:flex justify-center items-center space-y-4 md:space-y-0">
                <p class="font-bold text-red-600 md:text-lg text-sm text-center">
                    مجموعه : {{ $collection->name }} با کد {{ $collection->code }}
                </p>
            </div>
        </div>
