    ...
    <div class="mt-4 flex justify-between items-center space-x-4 space-x-reverse">
        <div>
            <p class="text-lg font-bold text-black">
                لیست قطعات مجموعه <span class="text-red-600">{{ $collection->name }}</span>
            </p>
        </div>
        <div>
                ...
                @foreach($collection->parts as $part)
                    <tr>
                        ...
                        <td class="px-4 py-3 space-x-3 space-x-reverse">
                            <form action="{{ route('collections.destroyPart',[$collection->id,$part->id]) }}"
                                  method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button class="form-cancel-btn text-xs"
                                        onclick="return confirm('قطعه از مجموعه حذف شود ؟')">
                                    حذف از مجموعه {{ $collection->name }}
                                </button>
                            </form>
                        </td>
                    </tr>
