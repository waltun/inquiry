<x-layout>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('collections.store') }}" class="md:grid grid-cols-2 gap-4 mt-4">
        @csrf
        ...
        <div class="bg-white shadow-sm p-4 rounded-md border border-gray-200 mb-4 md:mb-0">
            ...
            <div class="mt-4">
                ...
                <input type="text" id="inputName" name="name" class="input-text"
                       placeholder="مثال : فن هواساز" value="{{ old('name') }}">
            </div>

            <div class="mt-4">
                <label for="inputUnit" class="block mb-2 md:text-sm text-xs text-black">واحد مجموعه</label>
                <input type="text" id="inputUnit" name="unit" class="input-text" placeholder="مثال : دستگاه"
                       value="{{ old('unit') }}">
            </div>
