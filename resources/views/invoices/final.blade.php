    ...
    <x-slot name="js">
        ...
        <script>
            ...
            document.getElementById('delete-selected-btn').addEventListener('click', function (e) {
                if (confirm('پیش فاکتور ها حذف شوند ؟')) {
                    let selected = [];
                    document.querySelectorAll('input[name="invoices[]"]:checked').forEach(checkbox => {
                        selected.push(checkbox.value);
                    });

                    if (selected.length === 0) {
                        alert('هیچ پیش فاکتوری انتخاب نشده است.');
                        return;
                    }

                    let token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                    fetch('{{ route("invoices.final.delete-all") }}', {
                        method: 'POST',
                        headers: {
