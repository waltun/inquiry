<x-layout>
    ...
    <form method="POST" action="{{ route('exit-coding.update', [$exitt->id, $codingExit->id]) }}"
        ...
        @if(is_null($codingExit->coding_id) || request()->has('coding'))
            <input type="hidden" name="coding_id" value="{{ $selectedCoding->id ?? '' }}">
        @else
            @php
                $currentCoding = \App\Models\System\Coding::find($codingExit->coding_id);
            @endphp
                ...
                <tr class="text-xs border border-gray-400 text-center">
                    ...
                    <td class="p-1 border-r border-gray-400">
                        @if(is_null($codingExit->coding_id) || request()->has('coding'))
                            <input type="text" name="name" id="inputName"
                                   value="{{ $selectedCoding->name ?? $codingExit->name }}"
                    ...
                    <td class="p-1 border-r border-gray-400">
                        <input type="text" name="quantity" id="inputQuantity" class="input-text w-20 text-center"
                               placeholder="تعداد" value="{{ old('quantity', $codingExit->quantity) }}">
                    </td>
                    <td class="p-1 border-r border-gray-400">
                        @if(is_null($codingExit->coding_id) || request()->has('coding'))
