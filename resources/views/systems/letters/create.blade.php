<x-layout>
    ...
    <!-- Create User form -->
    <form action="{{ route('letters.store') }}" method="post" class="mt-11">
        @csrf

        <div class="grid grid-cols-3 gap-4 mb-4">
            @foreach($terms as $term)
                <a href="{{ route('letters.create') }}?term={{ $term->id }}"
            ...
            <div class="grid grid-cols-12 gap-4 items-center">
                ...
                <div class="col-span-10">
                    <select name="category" id="inputCategory" class="input-text">
                        <option value="">انتخاب کنید</option>
                        <option value="پیش فاکتور" {{ old('category') == 'پیش فاکتور' ? 'selected' : '' }}>
                            پیش فاکتور
                        </option>
                        <option value="معرفی نامه" {{ old('category') == 'معرفی نامه' ? 'selected' : '' }}>
                            معرفی نامه
                        </option>
                        <option value="مکاتبات اداری" {{ old('category') == 'مکاتبات اداری' ? 'selected' : '' }}>
                            مکاتبات اداری
                        </option>
                        <option
                            value="مکاتبات با مشتریان" {{ old('category') == 'مکاتبات با مشتریان' ? 'selected' : '' }}>
                            مکاتبات با مشتریان
                        </option>
                        <option value="اعلام پایان ساخت" {{ old('category') == 'اعلام پایان ساخت' ? 'selected' : '' }}>
                            اعلام پایان ساخت
                        </option>
                        <option value="سایر" {{ old('category') == 'سایر' ? 'selected' : '' }}>
                            سایر
                        </option>
                    </select>
                </div>
        ...
        <div class="card">
            ...
            <div class="grid grid-cols-12 gap-4 items-center">
                ...
                <div class="col-span-10">
                    <input type="text" name="date" id="inputDate" class="input-text" value="{{ $today }}">
                </div>
            </div>
        </div>
        ...
        @if(request()->has('term'))
            <div class="card">
                <div class="card-header">
                    <p class="card-title">متن</p>
                </div>

                <div class="mt-4">
                    <label for="inputDescription" class="form-label">متن نامه</label>
