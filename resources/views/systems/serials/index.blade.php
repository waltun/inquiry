        ...
        <table class="w-full border-collapse">
            ...
            @foreach($serials as $serial)
                <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                    ...
                    <td class="table-tr-td border-t-0 border-x-0">
                        {{ $serial->type == 'official' ? 'رسمی' : 'عملکردی' }}
                    </td>
                    <td class="table-tr-td border-t-0 border-x-0">
                        {{ $serial->number }}
                    </td>
                    ...
                    <td class="table-tr-td border-t-0 border-x-0">
                        ...
                        @else
                            {{ jdate($serial->send_date)->format('%A, %d %B %Y') }}
                        @endif
                    </td>
                        ...
                        <div class="table-parent-dropdown" x-data="{open:false}">
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                <form action="{{ route('serials.replicate',$serial->id) }}" method="POST"
                                    ...
                                    <button onclick="return confirm('شماره سریال کپی شود ؟')">
                                        کپی
                                    </button>
                                </form>
                                <a href="{{ route('serials.edit',$serial->id) }}"
                                ...
                                <form action="{{ route('serials.destroy',$serial->id) }}" method="POST"
                                    ...
                                    <button onclick="return confirm('شماره سریال حذف شود ؟')">
                                        حذف
                                    </button>
                                </form>
                            </div>
