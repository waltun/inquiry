<x-layout>
    ...
    <div class="flex items-center justify-between mt-8">
        ...
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('employees.create') }}" class="page-success-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
                <span class="mr-2">ایجاد کارمند جدید</span>
            </a>
        </div>
    </div>
            ...
            <table class="w-full border-collapse">
                ...
                @foreach($employees as $employee)
                    <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ jdate($employee->start_date)->format('Y/m/d') }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0">
                            @if(!is_null($employee->end_date))
                                {{ jdate($employee->end_date)->format('Y/m/d') }}
                            @else
                                مشغول به کار
                            @endif
                        </td>
                            ...
                            <div class="flex items-center space-x-4 space-x-reverse">
                                <form action="{{ route('employees.destroy', $employee->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="table-dropdown-delete"
                                            onclick="return confirm('کارمند حذف شود ؟')">
                                ...
                                <a href="{{ route('employees.edit', $employee->id) }}" class="table-dropdown-edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-1">
                                        <path stroke-linecap="round" stroke-linejoin="round"
