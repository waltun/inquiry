<x-layout>
    ...
    <div class="flex items-center justify-between mt-8">
        ...
        @can('create-category')
            <div>
                <a href="{{ route('system-categories.create') }}" class="page-success-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
    ...
    @if(!$categories->isEmpty())
        <div class="mt-4 space-y-4">
            @foreach($categories as $system_category)
                <div class="dashboard-cards group">
                    <div class="flex items-center space-x-4 space-x-reverse">
                        @if(count($system_category->children))
                        ...
                        <div class="space-x-4 space-x-reverse flex items-center">
                            <a href="{{ route('system-categories.children',$system_category->id) }}"
                            ...
                            <a href="{{ route('system-categories.children',$system_category->id) }}"
                               class="font-medium text-black text-xs group-hover:text-white dark:text-white">
                                تعداد کدینگ ها : {{ count($system_category->codings) }}
                            </a>
                            <span class="dark:text-white">|</span>
                            <a href="{{ route('system-categories.children',$system_category->id) }}"
                               class="font-medium text-black text-xs group-hover:text-white dark:text-white">
                                کد : {{ $system_category->code }}
                            </a>
                            <span class="dark:text-white">|</span>
                            <div x-data="{open:false}" class="flex relative">
                                ...
                                <div x-show="open" @click.away="open = false" x-cloak
                                    ...
                                    <a href="{{ route('system-categories.edit',$system_category->id) }}"
                                    ...
                                    <form action="{{ route('system-categories.destroy',$system_category->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
    ...
    @else
        <div class="bg-yellow-500 p-4 rounded-md shadow-md mt-4">
            <p class="text-sm font-bold text-black text-center">
                شما هنوز دسته بندی ای وارد نکرده اید!
            </p>
        </div>
    @endif
</x-layout>
