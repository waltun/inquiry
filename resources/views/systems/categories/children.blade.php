<x-layout>
        ...
        <div class="flex items-center">
            ...
            <div class="mr-2">
                <p class="breadcrumb-p-active">
                    لیست زیردسته های {{ $system_category->name }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between mt-8">
        <div class="flex items-center">
            ...
            <div class="mr-2">
                @if ($system_category->parent_id == 0)
                ...
                @else
                    <a href="{{ route('system-categories.children',$system_category->parent_id) }}"
                       class="category-back">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                             stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @foreach($system_category->children as $children)
            @php
                $status = true;
                $count = 0;
                $id = $children->id;
    ...
    <div class="mt-6 space-y-4">
        @foreach($system_category->children as $child)
            <div class="dashboard-cards group">
                <div class="flex items-center space-x-4 space-x-reverse">
                    ...
                    <div class="space-x-4 space-x-reverse flex items-center">
                        <a href="{{ route('system-categories.children',$child->id) }}"
                        ...
                        <a href="{{ route('system-categories.children',$child->id) }}"
                           class="font-medium text-black text-xs group-hover:text-white dark:text-white">
                            تعداد کدینگ ها : {{ count($child->codings) }}
                        </a>
                        <span class="dark:text-white">|</span>
                        <a href="{{ route('system-categories.children',$child->id) }}"
                           class="font-medium text-black text-xs group-hover:text-white dark:text-white">
                        ...
                        <div x-data="{open:false}" class="flex relative">
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                <a href="{{ route('system-categories.edit',$child->id) }}"
                                ...
                                <form action="{{ route('system-categories.destroy',$child->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
