<x-layout>
    ...
    <div class="md:flex items-center justify-between mt-8 space-y-4 md:space-y-0">
        ...
        <div class="flex items-center space-x-4 space-x-reverse">
            <a href="{{ route('leaves.create') }}" class="page-success-btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/>
                </svg>
                <span class="mr-2">ایجاد مرخصی جدید</span>
            </a>
        </div>
    </div>
            ...
            <table class="w-full border-collapse">
                ...
                @foreach($leaves as $leave)
                    <tr class="table-tb-tr group {{ $loop->even ? 'bg-sky-100' : '' }}">
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ $leave->end_date ? jdate($leave->end_date)->format('Y/m/d') : '-' }}
                        </td>
                        <td class="table-tr-td border-t-0 border-x-0">
                            {{ $leave->start_hour ?? '-' }}
                        </td>
                        ...
                        <td class="table-tr-td border-t-0 border-x-0">
                            @if($leave->confirm)
                                <p class="bg-green-500 text-white rounded-md px-4 py-1 text-xs">
                                    تایید شده
                                </p>
                            @else
                                <p class="bg-red-500 text-white rounded-md px-4 py-1 text-xs">
                                    منتظر تایید
                                </p>
                            @endif
                        </td>
                            ...
                            <div class="flex items-center justify-center space-x-4 space-x-reverse relative"
                                ...
                                <div x-show="open" @click.away="open = false" class="table-dropdown -top-16 -right-24"
                                    ...
                                    <form action="{{ route('leaves.destroy',$leave->id) }}" method="POST"
                                        ...
                                        <button onclick="return confirm('مرخصی حذف شود ؟')">
                                            حذف
                                        </button>
                                    </form>
                                </div>
                ...
                <div class="bg-white rounded-md shadow border border-indigo-500 relative">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-end space-x-2 space-x-reverse p-4">
                            @if($leave->type == 'daily')
                                <p class="table-info-btn text-xs">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-1">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                              d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z"/>
                            ...
                            <div class="flex items-center justify-center space-x-4 space-x-reverse">
                                @if($leave->start_date)
                                    <p class="text-xs font-medium text-black text-center">
                                        از تاریخ : {{ jdate($leave->start_date)->format('Y/m/d') }}
                                    </p>
                                @endif
                                @if($leave->end_date)
                                    <p>
                                    ...
                                    <p class="text-xs font-medium text-black text-center">
                                        تا تاریخ : {{ jdate($leave->end_date)->format('Y/m/d') }}
                                    </p>
                                @endif
                            </div>
                        @endif
