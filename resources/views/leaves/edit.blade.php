<x-layout>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('leaves.update', $leaf->id) }}" class="md:grid grid-cols-2 gap-4 mt-4">
        @csrf
        @method('PATCH')
        ...
        <div class="card">
            ...
            <div class="mt-4">
                ...
                <select name="type" id="inputType" class="input-text" onchange="showSections()">
                    ...
                    <option value="daily" {{ $leaf->type == 'daily' ? 'selected' : '' }}>
                        مرخصی روزانه
                    </option>
                    <option value="hourly" {{ $leaf->type == 'hourly' ? 'selected' : '' }}>
                        مرخصی ساعتی
                    </option>
                </select>
            </div>

            <div class="mt-4" id="startDateSection">
                ...
                <input type="text" id="inputStartDate" name="start_date" class="input-text"
                       placeholder="برای انتخاب تاریخ کلیک کنید" value="{{ old('start_date', $startDate) }}">
            </div>

            <div class="mt-4 {{ $leaf->type == 'hourly' ? 'hidden' : '' }}" id="endDateSection">
                <label for="inputEndDate" class="form-label">تا تاریخ</label>
                <input type="text" id="inputEndDate" name="end_date" class="input-text"
                       placeholder="برای انتخاب تاریخ کلیک کنید" value="{{ old('end_date', $endDate) }}">
            </div>

            <div class="mt-4 {{ $leaf->type == 'daily' ? 'hidden' : '' }}" id="startHourSection">
                ...
                <input type="number" id="inputStartHour" name="start_hour" class="input-text"
                       placeholder="ساعت را به عدد وارد کنید" value="{{ old('start_hour', $leaf->start_hour) }}">
            </div>

            <div class="mt-4 {{ $leaf->type == 'daily' ? 'hidden' : '' }}" id="endHourSection">
                <label for="inputEndHour" class="form-label">ساعت خاتمه</label>
                <input type="number" id="inputEndHour" name="end_hour" class="input-text"
                       placeholder="ساعت را به عدد وارد کنید" value="{{ old('end_hour', $leaf->end_hour) }}">
            </div>
        ...
        <div class="card">
            ...
            <div class="mt-4">
                ...
                <textarea name="description" id="inputDescription"
                          class="input-text resize-none h-32">{{ old('description', $leaf->description) }}</textarea>
            </div>
        </div>

        <div class="flex items-center space-x-4 space-x-reverse">
            <button type="submit" class="form-edit-btn" id="edit-button">
