...
class InquiryPartConverterController extends Controller
{
    public function index(Request $request, Inquiry $inquiry)
    {
        $request->validate([
            'converter_type' => 'required'
        ]);

        if ($request['converter_type'] == 'evaporator') {
            $part = Part::find('1194');
            return redirect()->route('inquiryPart.converter.evaporator', [$inquiry->id, $part->id]);
        }

        if ($request['converter_type'] == 'condensor') {
            $part = Part::find('1301');
            return redirect()->route('inquiryPart.converter.condensor', [$inquiry->id, $part->id]);
        }
    }

    public function evaporator(Inquiry $inquiry, Part $part)
    {
        $categories = Category::where('parent_id', 0)->get();
        return view('calculate.inquiry-converters.evaporator', compact('part', 'inquiry', 'categories'));
    }
    ...
    public function storeEvaporator(Request $request, Inquiry $inquiry, Part $part)
    {
        $request->validate([
    ...
    {
        $categories = Category::where('parent_id', 0)->get();
        return view('calculate.inquiry-converters.condensor', compact('part', 'categories', 'inquiry'));
    }
